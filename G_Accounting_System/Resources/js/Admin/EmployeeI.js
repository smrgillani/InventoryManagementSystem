$('.select2').select2();
$('.nav_tabs_for_new_employee > li > a').click(function () {
    if ($(this).attr("href") == "#v-general-dtls") {
        $('._Add_New_Employee_Form_Remover__').html("Cancel");
        $('#_Add_New_Employee_Form_Save_').html("Next");
        $('#_Update_Existing_Employee_Form_Save_').html("Next");
        setTimeout(function () {
    ...
    if ($(this).attr("href") == "#v-address-dtls") {
        $('._Add_New_Employee_Form_Remover__').html("Previous");
        $('#_Add_New_Employee_Form_Save_').html("Next");
        $('#_Update_Existing_Employee_Form_Save_').html("Next");
    ...
    if ($(this).attr("href") == "#v-payment-dtls") {
        $('._Add_New_Employee_Form_Remover__').html("Previous");
        $('#_Add_New_Employee_Form_Save_').html("Save");
        $('#_Update_Existing_Employee_Form_Save_').html("Update");
        setTimeout(function () { $('input[name=Bank_account_number]').focus(); }, 1);
    }
});

$("._Add_New_Employee_Form_Remover_").click(function () {
    EmployeeFormRemover();
});

$('#_Add_New_Employee_Form_Save_').on('click', function () {
    if ($(this).html() == "Next") {
        $('.nav-tabs > .active').next('li').find('a').trigger('click');
        if ($('.nav-tabs > .active > a').attr("href") == "#v-payment-dtls") {
            if ($(this).attr('id') == '_Update_Existing_Employee_Form_Save_') {
                $(this).html("Update");
            } else {
                $(this).html("Save");
            }
...
$('._Add_New_Employee_Form_Remover__').on('click', function () {
...
function EmployeeFormRemover() {
    $('._Add_New_Employee_Form_').slideUp("slow");
    $('._Add_New_Employee_Form_').html('');
    $("#_Add_New_Employee_btn_").attr("disabled", false);
...
$("#employee_image").change(function () {
    $("#base64").val("");
    var files = $("#employee_image").get(0).files;
    var file = files[0];
...
function InsertUpdateEmployee() {
    ...
    if ($("input[name=Full_name]").val() === "") {
        ...
        $("#_Add_New_Employee_Form_Save_").attr("disabled", true);
        var url = '/Employee/InsertUpdateEmployee';
        var employee = {
            id: $("#employeeid").val(),
        ...
        $.ajax({
            ...
            if (resp.pFlag == 1) {
                ...
                var Employee_id = resp.pContactid_Out;
                $('#_Error_Message_Display_ > p').html('');
                $('#_Error_Message_Display_').slideUp("slow");
...
//    if (event.which == 13) {
//        // Save Function
//        $('#_Add_New_Employee_Form_Save_').click();
...
//    if (event.which == 27) {
//        // Save Function
//        $('._Add_New_Employee_Form_Remover__').click();
//        event.preventDefault();
